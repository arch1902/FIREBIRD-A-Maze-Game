...
void Enemy::move_normal_enemy(Enemy_data &enemy, const Maze &maze,
  ...
  if (enemy.pos != dst_pos) {
    ...
    }else if (dst_pos.x < enemy.pos.x) {
      enemy.pos.x -= move_value;
    }
    if (dst_pos.y > enemy.pos.y) {
      enemy.pos.y += move_value;
    }else if (dst_pos.y < enemy.pos.y) {
      enemy.pos.y -= move_value;
    }
    return;
  }
...
void Enemy::move_lose_enemy(Enemy_data &enemy, const Maze &maze, const Player &p1,
  ...
  if (now_value
      > maze.get_home_distance(Point{enemy.block.x, enemy.block.y - 1})) {
    --enemy.next_block.y;
    return;
  }

  if (now_value
      > maze.get_home_distance(Point{enemy.block.x - 1, enemy.block.y})) {
    --enemy.next_block.x;
    return;
  }
