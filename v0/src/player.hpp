...
class Player {
 ...
 public:
  ...
  inline void draw(const game_mode mode) const  {
    switch (type_) {
      case player_type::p1: {
  ...
  inline void move(const Map &map, const game_mode mode)  {
  ...
  inline Point get_block() const  { return block_; }

  inline int get_life() const  { return life_; }

  inline void set_life(const int life)  { life_ = life; }

  inline unsigned int get_score() const  { return score_; }

  inline void set_score(const unsigned int score)  { score_ = score; }

  inline bool get_damaged() const  { return damaged_p_; }

  inline void set_damaged(const bool damaged_p)  {
    damaged_p_ = damaged_p;
  }

  inline unsigned int get_power_mode() const  { return power_mode_; }

  inline void set_power_mode(const unsigned int power_mode)  {
