...
class ImageManager {
    ...
    void load(const char *path, char image_type)  {
    ...
    public:
        ...
        SDL_Texture *get(char image_type) const  {
            return SDL_CreateTextureFromSurface(renderer_, images_[image_type]);
        }

        void render_copy(SDL_Texture &texture, const SDL_Rect &src,const SDL_Rect &dst) const  {
